...
def resnet50_vd_animals(ImagePath, top_k):
    # 检测路径是否为空
    if not ImagePath:
        logging.error(encode_str_by_sys("Image path may be empty"))
        return -1
    else:
        intput_path = "fuc/algorithm/feature/Image_classification/classification_of_animals/intput/"
    ...
    if not results:
        ...
        return -1
    # 删除缓存input图片
    path = "fuc/algorithm/feature/Image_classification/classification_of_animals/intput/"
...
def main(args):
    ...
    if is_linux or is_mac:
        ...
        # TODO 写你的算法函数
        res = resnet50_vd_animals(args.imagePath, args.top_k)
        # 下载
        # 处理
        # 返回结果
        print(res)
