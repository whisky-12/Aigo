...
def UGATIT_100w(content_imagePath):
    ...
    output_dir = "fuc/algorithm/feature/Image2Image/Cartoon_profile/output/"
    ...
    # fileName = 'output/'+str(time.time()).split(".")[0]+".jpg"
    # cv2.imwrite(fileName, results[0])

    path = "fuc/algorithm/feature/Image2Image/Cartoon_profile/intput/"
    # 删除缓存intput图片
    ...
    for pic in files:  # 遍历文件夹
        if pic.endswith(".png"):
            ...
            os.remove(path + '/' + pic)
    # 删除缓存output图片
    path = "fuc/algorithm/feature/Image2Image/Cartoon_profile/output/"
    files = os.listdir(path)
    for pic in files:  # 遍历文件夹
        if pic.endswith(".png"):
            os.remove(path + '/' + pic)
...
def main(args):
    ...
    if is_linux or is_mac:
        ...
        # TODO 写你的算法函数
        res = UGATIT_100w(args.imagePath)
        # 下载
        # 处理
        # 返回结果
        print(res)
