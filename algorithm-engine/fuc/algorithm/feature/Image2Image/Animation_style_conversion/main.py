...
def stylepro_artistic(content_imagePath, styles_imagePath, alpha):
    # 检测路径是否为空
    if not content_imagePath:
        logging.error(encode_str_by_sys("Image path may be empty"))
        return -1
    else:
        intput_path = "fuc/algorithm/feature/Image2Image/Animation_style_conversion/intput/"
    ...
    if not styles_imagePath:
        ...
        intput_path = "fuc/algorithm/feature/Image2Image/Animation_style_conversion/intput/"
        styles_imagePath = DownloadUtils.download_image(styles_imagePath,intput_path)
    ...
    try:
        ...
        return -1
    output_dir ="fuc/algorithm/feature/Image2Image/Animation_style_conversion/output/"
    ...
    cv2.imwrite(output_file, results[0].get('data'))
    uploadUrl = MathUtils.upload_image(timeMask+ ".png", output_file)
    # 删除缓存output图片
    path = "fuc/algorithm/feature/Image2Image/Animation_style_conversion/output/"
    ...
    for pic in files:  # 遍历文件夹
        if pic.endswith(".png"):
            ...
            os.remove(path + '/' + pic)
    path = "fuc/algorithm/feature/Image2Image/Animation_style_conversion/intput/"
    # 删除缓存intput图片
    files = os.listdir(path)
    for pic in files:  # 遍历文件夹
        if pic.endswith(".png"):
